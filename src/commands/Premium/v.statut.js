                ...
                if (reaction.message.id == msg.id) {
                    ...
                    if (reaction.emoji.name === '2️⃣') {
                        ...
                        const filter2 = (reaction, user) => {
                            return (['✔️', '❌'].includes(reaction.emoji.id) || ['📺', '🎮'].includes(reaction.emoji.name)) && user.id === message.author.id;
                        };
                        ...
                        collector.on('collect', async (reaction, user) => {
                            ...
                            if (reaction.message.id == msgg.id) {
                                ...
                                if (reaction.emoji.name === '🟣') { // Listening
                                    await reaction.users.remove(user.id)
                                ...
                                if (reaction.emoji.id === '🎧') { // Streaming
                                    await reaction.users.remove(user.id)
                                    if (db.get(`${client.user.id}.activityTypeTime`) === true) {
                                        message.channel.send(`${lang.SetUsernameFiveHour}`).then((mssg) => mssg.delete({
                                            timeout: 5000,
